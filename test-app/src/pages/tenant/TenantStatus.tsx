...
export default function TenantStatus() {
  ...
  useEffect(() => {
    const fetchChecklists = async () => {
      ...
      setChecklists(
        data.checklists.map((item: Omit<ChecklistItem, 'equipment'> & { equipment: string }) => ({
          ...item,
          equipment: item.equipment ? JSON.parse(item.equipment) as EquipmentItem[] : [],
        }))
      );
      setTotal(data.total);
      setLoading(false);
    ...
    fetchChecklists();
  }, [currentPage, filterType, filterDate, filterBuilding, filterUnit, filterTechnician, orderBy]);

 console.log('roleid:', roleid, 'tenantName:', tenantName);
 console.log('checklists:', checklists);
 const filteredChecklists = checklists
  .filter(item => {
    if (roleid === 'TENANT' && tenantName) {
        return item.tenant.trim().toLowerCase() === tenantName.trim().toLowerCase();
    }
    ...
    return (
      <TenantView
        reportData={{
          ...
          submissionDate: new Date(selectedChecklist.submissionDate),
          tenantSignature: selectedChecklist.tenantSignature, // Fill with actual data if available
          technicianSignature: selectedChecklist.technicianSignature, // Fill with actual data if available
          equipment: selectedChecklist.equipment,
          images: 0, // Fill with actual data if available
          videos: 0, // Fill with actual data if available
            Reference: getRefNum({
            visitType: selectedChecklist.visitType,
            unit: selectedChecklist.unit,
            submissionDate: new Date(selectedChecklist.submissionDate)
        ...
        selectedEquipment={selectedChecklist.equipment} // Fill with actual data if available
...
return (
  ...
  <div className="w-full p-4 dark:bg-gray-800">
    ...
    <div className="flex items-center justify-between mb-6">
      <h1 className="text-2xl font-bold">Tenant Status Reports</h1>

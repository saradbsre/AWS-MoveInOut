...
const styles = StyleSheet.create({
  ...
  tableContainer: {
  breakInside: 'auto', // Allow table to break across pages
  breakBefore: 'auto',
  breakAfter: 'auto',
},
signatureSection: {
...
const ReportPDF: React.FC<ReportPDFProps> = ({ reportData, selectedEquipment, username, barcodeBase64 }) => (
  <Document>
    ...
    <View style={styles.titleBarcodeRow}>
      ...
      <Text style={styles.title}>Tenant Report</Text>
      {/* {barcodeBase64 && (
      ...
      <View style={styles.row}>
        <Text style={[styles.cell, styles.label, styles.col10]}>Tenant</Text>
        <Text style={[styles.cell, styles.col90]}>{reportData.tenant}</Text>
      </View>
      <View style={styles.row}>
        <Text style={[styles.cell, styles.label, styles.col10]}>Building</Text>
        <Text style={[styles.cell, styles.col90]}>{reportData.building}</Text>
      </View>
      <View style={styles.row}>
        <Text style={[styles.cell, styles.label, styles.col10]}>Date</Text>
        <Text style={[styles.cell, styles.col40]}>
          {formatDateTimeLong(reportData.submissionDate)}
        </Text>
        <Text style={[styles.cell, styles.label, styles.col10]}>Visit Type</Text>
        <Text style={[styles.cell, styles.col40]}>{reportData.visitType}</Text>
      </View>
      <View style={styles.row}>
        <Text style={[styles.cell, styles.label, styles.col10]}>Contract No</Text>
        <Text style={[styles.cell, styles.col40]}>{reportData.contractNo}</Text>
        <Text style={[styles.cell, styles.label, styles.col10]}>Unit</Text>
        <Text style={[styles.cell, styles.col40]}>{reportData.unit}</Text>
      </View>
      <View style={styles.row}>
        <Text style={[styles.cell, styles.label, styles.col10]}>Start</Text>
        <Text style={[styles.cell, styles.col40]}>
          {reportData.startDate ? formatDateShort(reportData.startDate) : '-'}
        </Text>
        <Text style={[styles.cell, styles.label, styles.col10]}>End</Text>
        <Text style={[styles.cell, styles.col40]}>
      ...
      <View style={styles.tableContainer}>
        {selectedEquipment.length === 0 ? (
          ...
          selectedEquipment.map((item, idx) => (
            <View 
              style={styles.tableRow} // Remove the inline style with breakInside
              key={item.id} 
            >

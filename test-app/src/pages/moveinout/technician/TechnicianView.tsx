...
import * as ChecklistApi from '@/services/Transaction/Checklist/Checklistapi';
...
export default function TenantView({ reportData, selectedEquipment, onNewChecklist, fromHistory }: ReportViewProps) {
  ...
  // const [headerImageBase64, setHeaderImageBase64] = useState('');

// Replace the createPageFooter function with this version:
...
const handleSendEmail = async () => {
  ...
  // a.href = url;
  // a.download = `Checklist_Report_${reportData.contractNo}.pdf`;
  ...
  const GenerateEstimationCost = async () => {
    try {
      setIsSending(true);   
      // Map selectedEquipment to required fields
      const equipmentPayload = selectedEquipment.map((item, idx) => ({
      itemno: item.itemno,
      ...
      counter: idx + 1, // optional, if needed by backend
    }));
      const response = await ChecklistApi.GenerateEstimationCostApi(username, reportData.Reference, equipmentPayload);
      if (response.success) {
        setModalMessage("Estimation cost generated successfully!");
        setModalType("success");
        setShowModal(true);
      } else {
        throw new Error(response.message || "Failed to generate estimation cost.");
      }
    } catch (error) {
      console.error('Error generating estimation cost:', error);
      setModalMessage("Failed to generate estimation cost. Please try again.");
      setModalType("error");
      setShowModal(true);
    } finally {
      setIsSending(false);
...
return (
  <>
    {showModal && (
      ...
      Reference No: <span className="font-mono">{reportData.Reference}</span>
      </div>
          {/* Report Table */}
    ...
    <div className="no-print flex flex-col sm:flex-row gap-4 justify-center pt-6">
      ...
      {/* <button
        type="button"
        onClick={GenerateEstimationCost}
        disabled={isSending}
        className="flex items-center justify-center gap-2 px-6 py-3 bg-white hover:bg-green-700 text-black font-medium rounded-lg border border-grey-300 dark:text-white dark:bg-gray-700 dark:hover:bg-green-700 dark:border-gray-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isSending ? (
